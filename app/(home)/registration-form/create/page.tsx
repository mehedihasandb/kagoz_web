...
const MembershipFormInsert = () => {
      ...
      setChildrenRows(
        children.length > 0
          ? children?.map((child) => ({
            ...
            isActive: child.relativeIsActive ?? false,
          }))
          : [
            {
              id: null,
  ...
  useEffect(() => {
    if (customerBasicData) {
      ...
      setSerialNo(customerBasicData.serialNo);
      setApplicationDate(
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      const res = await updateMember(values).unwrap();
...
export default MembershipFormInsert;
